...
A programmatic UIKit reference app: each system control is built in code and shown with its public properties exercised, so you can see how Apple's UI actually looks. No third-party libraries, no `UIControl` subclasses, no kitchen-sink extensions.
